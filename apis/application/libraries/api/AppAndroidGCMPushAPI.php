...
class appAndroidGCMPushAPI {
    ...
    function __construct($args=NULL) {
		...
		// -------------------------
		$gcm_push_data = parse_ini_file('./resource/config.ini', true);
		//$phyicalwebroot = realpath($_SERVER['DOCUMENT_ROOT'].$gcm_push_data['Website']['VirtualFolder']);
		// $phyicalwebroot = realpath($_SERVER['DOCUMENT_ROOT'].$gcm_push_data['Website']['VirtualFolder']);
		$phyicalwebroot = $gcm_push_data['Website']['WebRoot'];
	...
	private function _pushMessage($push_message_id, $message, $device_reg_id, $push_mode, $user_id, $device_uid, $appname){
		...
		//echo '<pre>'; print_r($arrDecodeJsonMessage); exit;
		...
		$newMessageArr = array_merge($arr_registration_ids, $arrDecodeJsonMessage,  $arr_time_to_live, $arr_collapse_key);
		//$newMessageArr = array_merge($arr_registration_ids, $arrDecodeJsonMessage);
		$modifiedJsonMessage = json_encode($newMessageArr);
		//echo $modifiedJsonMessage; exit;
		//echo $this->gcmData[$push_mode]['serverApiKey'];exit;
		$registrationIds = array( $device_reg_id );
		//echo '<pre>'; print_r($fields_json); exit;
		try {
			$headers = array(
			...
			//echo '<pre>'; print_r($push_response);exit;
			//echo $modifiedJsonMessage; exit;
			curl_close($ch);
			$push_status = $this->_verifyPushResponse($push_response, $user_id, $push_message_id, $device_uid, $device_reg_id, $appname);
			return $push_status;
		}
		catch (Exception $e) {
	...
	private function _pushSuccess($push_message_id){
		//$sql = "DELETE FROM ".$this->tbl_push_message." WHERE `id`='{$push_message_id}'";
	...
	public function instaPushMessages($push_id){
		// only send one message per user... oldest message first
	    $sql = "SELECT `pm`.`id`, `pm`.`message`,`md`.`device_token`,`md`.`push_mode`,
	    `pm`.`fk_receiver_user_id`, `md`.`device_uid` 
			...
			if($result->num_rows) 
			{
				 $row = $result->fetch_array(MYSQLI_ASSOC);
					...
					*/
					...
					$push_status = $this->_pushMessage($push_message_id, $message, $device_reg_id, $push_mode, $user_id, $device_uid, $appname);
					return $push_status;
...
//		}
	...
	// Sample For Using addMessageVibrate()
	// <code>
	// $db = new DbConnect();
	// $db->show_errors();
	// $gcm = new GCM($db);
	// $gcm->addMessageVibrate(1, '2010-01-01 00:00:00');
	// $gcm->addMessageSound('bingbong.aiff'); // STRING OF FILE NAME
	// $gcm->queueMessage();
	// </code>
