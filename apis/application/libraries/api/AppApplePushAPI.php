...
class AppApplePushAPI {
	...
	function __construct($args=NULL) {
		...
		// -------------------------
		$apns_push_data = parse_ini_file('./resource/config.ini', true);
		//echo '<pre>';print_r($apns_push_data); exit;
	...
	private function _triggerError($error, $type=E_USER_NOTICE){
		...
		//echo '<pre>'; print_r($errorcode); exit;
	...
	private function _unregisterDevice($user_id=NULL, $appname, $device_uid, $device_token){
		if ($user != NULL) {
			$sql = "UPDATE ".$this->tbl_mobile_device." SET `status`='uninstalled' WHERE `device_token`='{$device_token}' AND `user_id`={$user_id} AND 
					`appname`='{$appname}' AND `device_uid`='{$device_uid}';";
			$this->mysqli->query($sql);
			$this->mysqli->commit();
		}
		 else {
			$sql = "UPDATE ".$this->tbl_mobile_device." SET `status`='uninstalled' WHERE `device_token`='{$device_token}' AND `appname`='{$appname}' AND 
	...
	private function _pushMessage($push_message_id, $message, $device_token, $push_mode, $user_id, $device_uid, $appname){
		...
		// echo '<pre>'; print_r($errorString);
		// echo '<pre>'; print_r($this->apnsData);
		// echo '<pre>'; print_r($push_mode);exit;
		if(!$fp){
	...
	private function _checkFeedback($push_mode, $user_id=NULL, $device_uid, $appname){
		$ctx = stream_context_create();
		stream_context_set_option($ctx, 'ssl', 'local_cert', $this->apnsData[$push_mode]['certificate']);
		stream_context_set_option($ctx, 'ssl', 'passphrase', 'Mass4Pass2013'); //$this->apnsData[$push_mode]['certificate_password']
	...
	private function _pushSuccess($push_message_id){
		//$sql = "DELETE FROM `tbl_push_messages` WHERE `push_message_id`='{$push_message_id}'";
                $sql = "UPDATE ".$this->tbl_push_message." SET `status`='D' WHERE `id`='{$push_message_id}' LIMIT 1;";
		$this->mysqli->query($sql);
                $this->mysqli->commit();
                return 1;
	}
	...
	private function _pushFailed($push_message_id){
		$sql = "UPDATE ".$this->tbl_push_message."
				SET `status`='F'
	...
	public function CronPushMessage($push_message_id, $message, $device_token, $push_mode, $user_id, $device_uid, $appname){
		$this->_pushMessage($push_message_id, $message, $device_token, $push_mode, $user_id, $device_uid, $appname);
		//$this->instaPushMessages($push_message_id);
	}

	//Method: InstaPushMessages() - Instantly Pushes a Message
	public function instaPushMessages($push_id){
		// only send one message per user... oldest message first
		 $sql = "SELECT `pm`.`id`, `pm`.`message`, `pm`.`fk_receiver_user_id`, `md`.`device_token`, `md`.`push_mode`,`md`.`device_uid` 
				FROM ".$this->tbl_push_message." as pm 
				LEFT JOIN ".$this->tbl_mobile_device." as md ON
				`md`.`device_uid` = `pm`.`device_uid` 
				WHERE `pm`.id='".$push_id."' ";
		if($result = $this->mysqli->query($sql)) 
		{
			...
			if($result->num_rows) 
			{
				 $row = $result->fetch_array(MYSQLI_ASSOC);
					...
					$push_status = $this->_pushMessage($push_message_id, $message, $device_token, $push_mode, $user_id, $device_uid, $appname);
					return $push_status;
 ...
 //                         $device_id = $this->push_message['send']['device_id'];
	...
	// 		 $sql="INSERT INTO `tbl_member_push_messages` (`fk_receiver_member_id`, `push_routing_mode`, `browser_session_id`, `device_uid`, `message`, `delivery`, `status`, `created`, `send_retry`, `retry_count`, `last_retry_attempt_datetime`, `modified`,`attend_mode`) VALUES 
 //                            ($user_id, 'D', NULL, '".$device_id."', '".$this->mysqli->real_escape_string($message)."', '".$delivery."', 'Q', CURRENT_TIMESTAMP, '0', NULL, NULL, CURRENT_TIMESTAMP,'C');";
 ...
 //                        $this->mysqli->query($sql);
    ...
    public function queueMessage(){
		//echo '<pre>'; print_r($this->push_message); exit;
		// check to make sure a message was created
		if(!isset($this->push_message)) $this->_triggerError('NOTICE: You cannot Queue a message that has not been created. Use newMessage() to create a new message.');
		...
		$pushalert = '1';
		$pushsound = '1';
		$sql = "SELECT `badge_count`,id FROM ".$this->tbl_mobile_device." WHERE `device_uid`='".$this->push_message['send']['device_id']."' ";
		...
		if(!$deliver) {
			...
			*/
			...
			//echo '<pre>'; print_r($this->push_message); exit;
			if($user_id>0){
				$sql="INSERT INTO ".$this->tbl_push_message." (`device_uid`, `fk_receiver_volunteer_id`,`message`, `delivery`, `status`,`attend_mode`, `created`, `send_retry`, `retry_count`, `last_retry`, `modified`) VALUES 
                            ('".$device_id."','".$user_id."','".$this->mysqli->real_escape_string($message)."', '".$delivery."', 'Q', 'C', CURRENT_TIMESTAMP, '0', NULL, CURRENT_TIMESTAMP,NULL);";
            }else{
            	$sql="INSERT INTO ".$this->tbl_push_message." (`device_uid`,`message`, `delivery`, `status`,`attend_mode`, `created`, `send_retry`, `retry_count`, `last_retry`, `modified`) VALUES 
                            ('".$device_id."','".$this->mysqli->real_escape_string($message)."', '".$delivery."', 'Q', 'C', CURRENT_TIMESTAMP, '0', NULL, CURRENT_TIMESTAMP,NULL);";
	...
	public function queueMessageDirect(){
		...
		if(!$deliver) {
			...
			*/
			...
			//$sql = "INSERT INTO `tbl_member_push_messages` VALUES (UUID(), {$user_id}, '".$this->mysqli->real_escape_string($message)."', '{$delivery}', 'Q', NOW(), NOW());";
			//echo '<pre>'; print_r($this->push_message); exit;
			$sql="INSERT INTO ".$this->tbl_push_message." (`device_uid`, `fk_receiver_user_id`,`message`, `delivery`, `status`,`attend_mode`, `created`, `send_retry`, `retry_count`, `last_retry`, `modified`) VALUES 
                            ('".$device_id."','".$user_id."','".$this->mysqli->real_escape_string($message)."', '".$delivery."', 'Q', 'D', CURRENT_TIMESTAMP, '0', NULL, CURRENT_TIMESTAMP,NULL);";
                        
			$this->mysqli->query($sql);
			$insert_id=$this->mysqli->insert_id;
			$this->mysqli->commit();
                     
			$push_status = $this->instaPushMessages($insert_id);
			return $push_status;
		}
	}
